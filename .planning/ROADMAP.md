...
**Success Criteria** (what must be TRUE):
  ...
  3. A simulated Anthropic 429 error sends a graceful fallback message to the lead and logs the event for follow-up — without crashing the server
...
**Success Criteria** (what must be TRUE):
  ...
  2. Structured pino logs for a full lead interaction — webhook receipt, Claude call, Digisac send, handoff trigger — are readable and filterable in Railway log viewer
